            ...
            try:
                ...
                if 'out of memory' in str(e).lower():
                    ...
                    try:
                        ...
                        pass
                    # Explicitly drop iter-local refs so the partial autograd
                    # graph is released NOW rather than living until the next
                    # iter overwrites its variables (which is what causes the
                    # repeat-OOM pattern). `del` requires the names to be
                    # bound; the None-pre-init at the top of the iter body
                    # guarantees that.
                    ...
                    del F0_real, F0, N_real, y_rec_gt, y_rec_gt_pred
                    del F0_fake, N_fake, y_rec
                    del loss_F0_rec, loss_norm_rec, d_loss, loss_mel
                    del loss_gen_all, loss_lm, loss_ce, loss_dur
                    del loss_s2s, loss_mono, g_loss
